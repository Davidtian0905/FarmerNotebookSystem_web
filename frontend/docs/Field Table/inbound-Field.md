...
#### 1. 图片上传与保存
- **保存路径**: `frontend/public/Inbound/`
- **文件命名规则**: 
  - 第一张图片（缩略图）: `image0.png`
  - 后续图片: `image1.png`, `image2.png`, `image3.png`...
  - 所有图片统一转换为PNG格式
- **支持格式**: JPG, PNG, GIF, WebP（上传后统一转换为PNG）
- **文件大小限制**: 5MB
- **图片预览**: 支持多图片预览和删除
- **缩略图功能**: 第一张上传的图片自动作为物料缩略图
...
#### 4. 物料编码自动生成
- **生成规则**: M_ + 物料名称拼音首字母 + 日期 + 序号
- **中文转拼音**: 内置拼音映射表
- **示例**: 铁观音 → M_TGY20250115001
- **前缀说明**: 所有物料编码统一以"M_"开头，便于识别和管理
